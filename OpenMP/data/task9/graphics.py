...
def plot_graphs(csv_file):
    ...
    for _, size in matrix_sizes.iterrows():
        ...
        time_highly_parallel_1 = subset[subset["NumThreads"] == 1]["ExecutionTimeHighlyParallel"].iloc[0]

        # Построение графика времени выполнения
        plt.figure(figsize=(10, 6))
        plt.plot(x, subset["ExecutionTime"], label="ExecutionTime", marker="o")
        plt.plot(x, subset["ExecutionTimeHighlyParallel"], label="ExecutionTimeHighlyParallel", marker="s")
        ...
        speedup_highly_parallel = time_highly_parallel_1 / subset["ExecutionTimeHighlyParallel"]

        plt.figure(figsize=(10, 6))
        plt.plot(x, speedup, label="Speedup", marker="o")
        plt.plot(x, speedup_highly_parallel, label="SpeedupHighlyParallel", marker="s")
...
csv_file = "t9_results.txt"  # Укажите путь к вашему CSV-файлу
